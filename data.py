...
def memory(sequence_length=1000, n_sequence=1, memory_delay=1, random_state=None):
    ...
    input_data = random_state.normal(loc=0., scale=1, size=(sequence_length, n_sequence))
...
def xor(sequence_length=1000, n_sequence=1, memory_delay=1, random_state=None):
    random_state = check_random_state(random_state)
    input_data = random_state.randint(low=0, high=2, size=(sequence_length, n_sequence))

    shifted_input_data = np.roll(input_data, 1, axis=0)
    output_data = np.logical_xor(input_data, shifted_input_data)
    y = np.roll(output_data, memory_delay, axis=0)
...
def narma(sequence_length=1000, n_sequence=1, random_state=None):
    random_state = check_random_state(random_state)
    input_data = random_state.uniform(high=0.5, size=(sequence_length, n_sequence))
    y = np.zeros((sequence_length, n_sequence))
...
def mackey_glass(sequence_length=1000, n_sequence=1, tau=17, random_state=None):
    ...
    input_data = np.zeros((sequence_length, n_sequence))
    input_data[:memory_length, :] = 1.1 + 0.2 * random_state.normal(loc=0., scale=1, size=(memory_length, n_sequence))
